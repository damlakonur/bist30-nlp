...
def getTweetsFromUser(api, user, since=None, until=None, lang="tr"):
    ...
    try:
        tweets = tweepy.Cursor(
            api.user_timeline, screen_name=user, since=since, until=until, lang=lang
        ).items(20000)
        for tweet in tweets:
            df = df.append(
                pd.Series(
...
def main():
    ...
    hastags = (
        ...
        " #EKGYO OR #EREGL OR #FROTO OR #GUBRF OR #SAHOL OR #HEKTS OR #KRDMD"
    )
    # Query length limit is 128 characters for search_full_archive,
    # 256 for search_30day
    a = (
        "OR #KCHOL OR #KOZAL OR #KOZAA OR #PGSUS OR #SASA OR #PETKM OR #TAVHL OR"
        " #TKFEN OR #TOASO OR #TCELL OR #TUPRS OR #THYAO OR #TTKOM OR #GARAN OR"
        " #HALKB OR #ISCTR OR #SISE OR #VESTL OR #YKBNK"
    )
